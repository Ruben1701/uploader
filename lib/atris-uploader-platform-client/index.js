...
module.exports = function (config, deps) {
  ...
  return {
    ...
    */
    ...
    /**
     * Upload device data for the given user to Atris
     *
     * @param {String} userId of the user to get the device data for
     ...
     */
     uploadDeviceDataForUser: function (userId, data, cb) {
      ...
      //  superagent
      //   .post(common.makeUploadUrl('/api/v1/users/diabetes'))
      //   .send(data)
      //   .set("X-Api-Key", user.getUserToken())
      //   .set(common.TRACE_SESSION_HEADER, common.getSessionTrace())
      //   .retry()
      //   .end(
      //   function (err, res) {
    ...
    /**
     * Upload  data for the given user to Atris
     *
     * @param {String} userId of the user to get the device data for
     * @param {Object} data to be uploaded (already cbg filtered)
     * @param cb
     * @returns {cb}  cb(err, response)
     */
     uploadDataToAtris: function (data, cb) {
      //common.assertArgumentsSize(arguments, 2);
