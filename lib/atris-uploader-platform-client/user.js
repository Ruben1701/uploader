...
module.exports = function (common, config, deps) {
   ...
   */
  function saveSession(newUserId, newToken, newName, options) {
    options = options || {};
    myToken = newToken;
    common.syncToken(myToken);
    myUserId = newUserId;
  ...
  /**
   * Login user to the Atris platform
   *
   * @param user object with a username and password to login
   * @param options (optional) object with `remember` boolean attribute
   * @param cb
   ...
   */
   function login(user, options, cb) {
    ...
    superagent
      .post(common.makeAPIUrl('/api/v1/login'))
      .type('form')
      ...
      function (err, res) {
        ...
        var theUserId = res.body.user.id;
        //var theToken = res.headers[common.SESSION_TOKEN_HEADER];
        var theToken = res.body.user.token;
        var theName = res.body.user.name;
        var FakedUser = {emailVerified: true, emails: [res.body.user.email], termsAccepted: "2021-06-28T18:58:22+02:00", userid: res.body.user.id, username: res.body.user.name};
        
        console.log(theUserId + " : " + theToken + " : " + theName);

        saveSession(theUserId, theToken, theName, options);
        return cb(null,{userid: theUserId, user: FakedUser});
      });
  }
