...
//wrapper for atris client local
import createTidepoolClient from '../atris-uploader-platform-client';
...
function createProfile(){
  var profile = {fullName: tidepool.getName() , patient: { birthday: null, diagnosisDate: null, diagnosisType: null, targetDevices:["medtronic", "carelink"], targetTimezone: "Europe/Amsterdam" } };
  return profile;
}
...
api.user.loggedInProfile = (cb) => {
  ...
  // });
  api.log(createProfile());
  return cb(null, createProfile());
};
...
api.user.getUploadGroups = (cb) => {
  ...
  //   (callback) => createProfile(callback),
  // ], (err, results) => {
  //   if (err) {
  //     cb(err);
  //   }
...
api.upload.getVersions = (cb) => {
  api.log('GET /info currently not implemented');
  // tidepool.checkUploadVersions((err, resp) => {
  //   if (err) {
  //     if (!navigator.onLine) {
  //       const error = new Error(errorText.E_OFFLINE);
...
api.upload.toPlatform = (data, sessionInfo, progress, groupId, cb, uploadType = 'jellyfish') => {
  ...
  api.log(grouped['cbg']);
  let cbgData = grouped['cbg'];
  
  cbgData.forEach( obj => renameKey( obj, 'deviceId', 'device_id' ));
  cbgData.forEach( obj => renameKey( obj, 'deviceTime', 'measured_at' ));

  //TODO dit moet eigenlijk in de back end afgehandeld worden dan kan grouped['cbg'] rechtstreeks doorgegeven worden.
  for (var i = 0; i < cbgData.length; i++) {
    var value = cbgData[i].value
    cbgData[i].measured_at = cbgData[i].measured_at.replace('T',' '); 
    delete cbgData[i].value
    delete cbgData[i].time
    delete cbgData[i].timezoneOffset
    delete cbgData[i].clockDriftOffset
    delete cbgData[i].conversionOffset
    delete cbgData[i].payload
    cbgData[i].value = value
}

  var test = {results: cbgData};
  api.log(test);

  return tidepool.uploadDataToAtris(test, cb);
...
api.upload.fetchCarelinkData = (payload, cb) => {
  api.log('POST /carelink');
  tidepool.uploadCarelinkDataForUser(payload, (err, syncTask) => {
    if (err) {
      return cb(err);
    }
